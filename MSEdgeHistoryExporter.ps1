...
$SelectedUser = Read-Host -Prompt "Enter username to export MS Edge history, leave blank to export current user ($env:UserName), or enter 'ALL' to export all users"
If ($SelectedUser -Match "ALL") {
    if ($null -ne $(Get-Process | Where-Object { $_.ProcessName -Like "*msedge" }) -and $(Test-Path -Path "C:\Users\$env:UserName\AppData\Local\Microsoft\Edge\User Data\Default\history" -PathType Leaf)) { $EdgeIsOpen = Read-Host -Prompt "Close edge, or press enter to close edge and proceed."; If ($null -eq $EdgeIsOpen -or $null -ne $EdgeIsOpen) { Write-Host "Closing Edge"; Get-Process | Where-Object { $_.ProcessName -Like "*msedge*" } | Stop-Process } }
    Write-Host "Exporting all users to Downloads folder."
    ForEach ($name in $LocalUsers) { If (Test-Path -Path "C:\Users\$name\AppData\Local\Microsoft\Edge\User Data\Default\history" -PathType Leaf) { $SelectedUser = $name; PullData } }; Break
}
